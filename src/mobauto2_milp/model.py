...
class MobautoMilpModel(_BaseModel):
    ...
    def _extract_solver_stats(self, res) -> dict[str, Any]:
        ...
        try:
            ...
            pass
        try:
            lower_bound = getattr(res.problem, "lower_bound", None)
            upper_bound = getattr(res.problem, "upper_bound", None)
            if lower_bound is not None:
                stats["best_bound"] = float(lower_bound)
            if upper_bound is not None:
                stats["incumbent"] = float(upper_bound)
        except Exception:
            pass
        try:
            if len(res.solution) > 0:
                gap = getattr(res.solution(0), "gap", None)
                if gap is not None:
                    stats["gap"] = float(gap)
        except Exception:
            pass
        try:
            ...
            if gap is not None and float(gap) > 1.0 and incumbent is not None and best_bound is not None:
                stats["gap"] = max(0.0, (float(incumbent) - float(best_bound)) / max(1.0, abs(float(incumbent))))
        except Exception:
            pass
        return stats
    ...
    def initialize(self) -> None:
        ...
        # These state variables are implied by the binary trip starts and location recursions.
        # Keeping them continuous avoids thousands of redundant binaries for large fleets.
        m.atL = pyo.Var(m.Q, m.T, bounds=(0.0, 1.0))
        m.atM = pyo.Var(m.Q, m.T, bounds=(0.0, 1.0))
    ...
    def solve(self) -> SolveResult:
        ...
        try:
            ...
            if mipgap is not None:
                mipgap_val = float(mipgap)
                if backend in ("cplex", ""):
                    solver.options["mip_tolerances_mipgap"] = mipgap_val
                else:
                    solver.options["mipgap"] = mipgap_val
